...
namespace ESoftor.Core.Options
{
    ...
    public class ESoftorOptionsSetup : IConfigureOptions<ESoftorOptions>
    {
        ...
        public void Configure(ESoftorOptions options)
        {
            ...
            //OAuth2
            section = _configuration.GetSection("ESoftor:OAuth2");
            ...
            //MailSender
            section = _configuration.GetSection("ESoftor:MailSender");
            MailSenderOptions sender = section.Get<MailSenderOptions>();
            if (sender != null)
            {
                if (sender.Password == null)
                {
                    sender.Password = _configuration["ESoftor:MailSender:Password"];
                }
                options.MailSender = sender;
            }

            //JwtOptions
            section = _configuration.GetSection("ESoftor:Jwt");
            JwtOptions jwt = section.Get<JwtOptions>();
            if (jwt != null)
            {
                if (jwt.Secret == null)
                {
                    jwt.Secret = _configuration["ESoftor:Jwt:Secret"];
                }
                options.Jwt = jwt;
            }

            // RedisOptions
            section = _configuration.GetSection("ESoftor:Redis");
            ...
            // SwaggerOptions
            section = _configuration.GetSection("ESoftor:Swagger");
        ...
        /// <summary>
        /// 初始化上下文配置信息，首先以ESoftor配置节点中的为准，
        /// 不存在ESoftor节点，才使用ConnectionStrings的数据连接串来构造SqlServer的配置，
        /// 保证同一上下文类型只有一个配置节点
        /// </summary>
        /// <param name="options"></param>
        private void SetDbContextOptions(ESoftorOptions options)
        {
            IConfigurationSection section = _configuration.GetSection("ESoftor:DbContexts");
            ...
            if (dict == null || dict.Count == 0)
            {
                ...
                {
                    DbContextTypeName = "ESoftor.EntityFrameworkCore.Defaults.DefaultDbContext,ESoftor.EntityFrameworkCore",
