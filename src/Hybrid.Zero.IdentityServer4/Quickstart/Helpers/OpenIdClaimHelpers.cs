...
namespace Hybrid.Zero.IdentityServer4.Quickstart.Helpers
{
    public static class OpenIdClaimHelpers
    {
        public static Claim ExtractAddressClaim(OpenIdProfile profile)
        {
            ...
            {
                addressJson[HybridClaimTypes.StreetAddress] = profile.StreetAddress;
            }

            if (!string.IsNullOrWhiteSpace(profile.Locality))
            {
                addressJson[HybridClaimTypes.Locality] = profile.Locality;
            }

            if (!string.IsNullOrWhiteSpace(profile.Region))
            {
                addressJson[HybridClaimTypes.Region] = profile.Region;
            }

            if (!string.IsNullOrWhiteSpace(profile.PostalCode))
            {
                addressJson[HybridClaimTypes.PostalCode] = profile.PostalCode;
            }

            if (!string.IsNullOrWhiteSpace(profile.Country))
            {
                addressJson[HybridClaimTypes.Country] = profile.Country;
            }
        ...
        public static OpenIdProfile ExtractProfileInfo(IList<Claim> claims)
        {
            ...
            try
            {
                ...
                if (addressJson.ContainsKey(HybridClaimTypes.StreetAddress))
                {
                    profile.StreetAddress = addressJson[HybridClaimTypes.StreetAddress].ToString();
                }

                if (addressJson.ContainsKey(HybridClaimTypes.Locality))
                {
                    profile.Locality = addressJson[HybridClaimTypes.Locality].ToString();
                }

                if (addressJson.ContainsKey(HybridClaimTypes.Region))
                {
                    profile.Region = addressJson[HybridClaimTypes.Region].ToString();
                }

                if (addressJson.ContainsKey(HybridClaimTypes.PostalCode))
                {
                    profile.PostalCode = addressJson[HybridClaimTypes.PostalCode].ToString();
                }

                if (addressJson.ContainsKey(HybridClaimTypes.Country))
                {
                    profile.Country = addressJson[HybridClaimTypes.Country].ToString();
                }
            }
