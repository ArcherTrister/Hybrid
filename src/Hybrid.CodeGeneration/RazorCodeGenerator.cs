// -----------------------------------------------------------------------
//  <copyright file="RazorCodeGenerator.cs" company="Hybrid开源团队">
...
using Hybrid.CodeGeneration.Schema;
using Hybrid.Data;
...
namespace Hybrid.CodeGeneration
{
    ...
    public class RazorCodeGenerator : ICodeGenerator
    {
        ...
        {
            string resName = $"Hybrid.CodeGeneration.Templates.{type.ToString()}.cshtml";
            Stream stream = GetType().Assembly.GetManifestResourceStream(resName);
            if (stream == null)
            {
                throw new HybridException($"无法找到“{type.ToString()}”的内置代码模板");
            }
