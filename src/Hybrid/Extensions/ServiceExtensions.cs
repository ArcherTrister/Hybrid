...
namespace Microsoft.Extensions.DependencyInjection
{
    public static partial class ServiceExtensions
    {
        ...
        /// 获取所有模块信息
        /// </summary>
        public static HybridModule[] GetAllPacks(this IServiceProvider provider)
        {
            return provider.GetServices<HybridModule>().OrderBy(m => m.Level).ThenBy(m => m.Order).ThenBy(m => m.GetType().FullName).ToArray();
        }
        ...
        //    HybridModule[] packs = provider.GetServices<HybridModule>().ToArray();
        //    foreach (HybridModule pack in packs)
        //    {
