...
namespace Test.Web.Conventions
{
    internal static class ModelConventionHelper
    {
        public static ICollection<SelectorModel> CreateSelectors(IList<object> attributes)
        {
            ...
            //
            // The set of route attributes are split into those that 'define' a route versus those
            // that are 'silent'.
            //
            // We need to define a selector for each attribute that 'defines' a route, and a single
            // selector for all of the ones that don't (if any exist).
            //
            // If the attribute that 'defines' a route is NOT an IActionHttpMethodProvider, then
            // we'll include with it, any IActionHttpMethodProvider that are 'silent'
            // IRouteTemplateProviders. In this case the 'extra' action for silent route providers
            // isn't needed.
            //
            // Ex: [HttpGet] [AcceptVerbs("POST", "PUT")] [HttpPost("Api/Things")] public void DoThing()
            //
            ...
            //
            // [Route("api/Products")] [AcceptVerbs("GET", "HEAD")] [HttpPost("api/Products/new")]
            //
            // This will generate 2 selectors:
            // 1. [AcceptVerbs("GET", "HEAD")]
            // 2. [HttpPost]
            //
            // Note that having a route attribute that doesn't define a route template _might_ be an
            // error. We don't have enough context to really know at this point so we just pass it on.
            ...
            {
                // If we see an attribute like [Route(...)]
                //
                // Then we want to group any attributes like [HttpGet] with it.
                //
                // Basically...
                //
                // [HttpGet] [HttpPost("Products")] public void Foo() { }
                //
                // Is two selectors. And...
                //
                // [HttpGet] [Route("Products")] public void Foo() { }
                //
                // Is one selector.
            ...
            {
                // Each of these routeProviders are the ones that actually have routing information
                // on them something like [HttpGet] won't show up here, but [HttpGet("Products")] will.
                foreach (var routeProvider in routeProviders)
                {
                    ...
                    foreach (var attribute in attributes)
                    {
                        ...
                        {
                            // Exclude other route template providers Example: [HttpGet("template")] [Route("template/{id}")]
                        }
                        else if (
                            routeProvider is IActionHttpMethodProvider &&
                            attribute is IActionHttpMethodProvider)
                        {
                            // Example: [HttpGet("template")] [AcceptVerbs("GET", "POST")]
                            //
                            // Exclude other http method providers if this route is an http method provider.
                        }
                        else
                        {
                            filteredAttributes.Add(attribute);
