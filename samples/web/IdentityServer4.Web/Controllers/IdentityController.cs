...
using ESoftor.Identity.JwtBearer.Dtos;
using ESoftor.Json;
...
namespace IdentityServer4.Web.Controllers
{
    [Description("网站-认证")]
    [ModuleInfo(Order = 1)]
    public class IdentityController : LocalApiController
    {
        ...
        public async Task<bool> CheckNickNameExists(string nickName)
        {
            IUserValidator<User> nickNameValidator =
                _userManager.UserValidators.FirstOrDefault(m => m.GetType() == typeof(UserNickNameValidator<User, Guid>));
        ...
        public async Task<AjaxResult> Login(LoginDto dto)
        {
            ...
            IUnitOfWork unitOfWork = HttpContext.RequestServices.GetUnitOfWork<User, Guid>();
            unitOfWork.Commit();
        ...
        {
            Guid userId = User.Identity.GetUserId<Guid>();
